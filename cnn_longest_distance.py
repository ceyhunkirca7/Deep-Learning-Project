...
# c ŞIKKINDA CNN MODELİ KULLANILMISTIR.
# veri üretim fonksiyonu
# bu fonk, 25x25 matris ve bu matristeki enuzak iki nokta arasındaki mesafeyi üretir
def generate_sample_c(size=25, min_points=3, max_points=10):
    ...
    for i in range(len(points)):
        for j in range(i + 1, len(points)):
            ...
            if dist > max_distance:
                max_distance = dist  # en uzak mesafe güncellenir
    return mat, max_distance  # matris ve en uzak mesafe döndürülür


# veri seti oluşturma fonksiyonu
# belirtilen sayıda örnek oluşturur
def generate_dataset_c(n_samples, size=25):
    X, y = [], []
    for _ in range(n_samples):
        mat, max_dist = generate_sample_c(size)
        X.append(mat)
        y.append(max_dist)
...
# veri oluşturma
X_train, y_train = generate_dataset_c(800)  # eğitim verisi oluşturulur
X_test, y_test = generate_dataset_c(200)  # test verisi oluşturulur
...
def visualize_predictions_c(model, X_test, y_test):
...
visualize_predictions_c(model, X_test, y_test)
